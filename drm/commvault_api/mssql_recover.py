...
logger = logging.getLogger('MSSQL_recover')
    ...
    try:
        ...
        try:
            ...
            if mssqlOverWrite == "True":
                overWrite = True
        # 需要恢复的数据库
        mssql_dbs = cvAPI.get_mssql_db(pri.client_id if pri else "", instance)

        jobId = cvAPI.restoreMssqlBackupset(pri_name, std_name, instance, {
            'overWrite': overWrite, "restoreTime": processrun.recover_time, "mssql_dbs": mssql_dbs
        })
        # jobId = 4553295
        if jobId == -1:
            print("SQL Server恢复接口调用失败，{0}。".format(cvAPI.msg))
            exit(1)
        else:
            temp_tag = 0
            waiting_times = 0
